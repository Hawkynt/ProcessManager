...
internal sealed class WindowsIdentityResolver {
  ...
  private readonly record struct Resolved(ulong StartTicks, string? Name, int RelativeId);
  ...
  /// </summary>
  public string? Resolve(int pid, ulong startTicks, out int userId) {
    userId = -1;
    ...
    this._byPid[pid] = new(startTicks, name, sid.Value.RelativeId);
