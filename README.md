...
A process explorer, a resource monitor and a performance grapher are usually three programs that
disagree with each other. ProcessManager is one: the same sampled data behind the tree, the columns
and the plots, so a number in one place means the same thing in the others.

The bias is toward being honest about measurement. Rates come from differencing counters over an
interval, a process that lives and dies inside one interval is a gap rather than a zero, and the UI
says which it is. `docs/PRD.md` tracks the rest of the surface box by box.

## ✨ Features

- The process tree with per-process CPU, memory, I/O, handles and threads
- In-cell history plots — CPU, memory and I/O drawn one pixel per sample, on a shared scale so rows compare
- One filter syntax, identical in the window, in the terminal and on the command line
- Privileged operations kept explicit rather than silently attempted
- A terminal UI and a desktop UI over the same data

## 📦 Installation

Download the binary from the [latest release](../../releases/latest) (or a `nightly-*` prerelease).
Reading other users' processes needs the usual privileges; see
